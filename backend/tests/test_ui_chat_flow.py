...
async def test_ui_chat_flow_screenshot_sequence(db_session):
    ...
    try:
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            assert "support ticket" in b3["message"].lower() or "reply with 1 or 2" in b3["message"].lower()
    finally:
...
async def test_ui_chat_flow_ticket_creation_sequence(db_session):
    ...
    try:
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            # 1) Complaint should offer support options
            r1 = await client.post(
                "/api/chat",
            ...
            assert b1["metadata"].get("offered_options") is True
            session_id = b1["session_id"]

            # 2) Choose option 1 (create ticket)
            r2 = await client.post(
                "/api/chat",
            ...
            b2 = r2.json()
            assert b2["intent"] == "complaint"
            assert "email" in b2["message"].lower()
            assert b2["metadata"] is not None
            assert b2["metadata"].get("awaiting_email") is True

            # 3) Provide email -> ticket created
            r3 = await client.post(
                "/api/chat",
            ...
            assert "ticket_id" in b3["metadata"]
            assert b3["metadata"].get("has_email") is True
    finally:
        app.dependency_overrides.clear()
